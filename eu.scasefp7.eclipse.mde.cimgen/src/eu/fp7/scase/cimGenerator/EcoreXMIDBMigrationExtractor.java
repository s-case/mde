...
public class EcoreXMIDBMigrationExtractor {
	...
	public EcoreXMIDBMigrationExtractor(String strOutputFolder, String strProjectName){
	...
	public void exportEcoreXMI(AnnotationModel oDBMigratorCIM){
			this.oEcoreResource.getContents().add(oDBMigratorCIM);
		
		//save to disk
		try {
			this.oEcoreResource.save(Collections.EMPTY_MAP);
		} catch (IOException e) {
			Activator.log("Could not export DB Migrator CIM to XMI file.", e);
		}
	}
